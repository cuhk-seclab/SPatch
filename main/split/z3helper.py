...
class Z3Helper:
    ...
    def isImply(self, cc1, cc2):
        ...
        z3fstr = self.__genImplyZ3(symTable, z3const1, z3const2)
        z3filename = self.__saveToPyFile(z3fstr)
        return self.runPyFile(z3filename)
    ...
    def __genImplyZ3(self, symTable, z3const1, z3const2):
        z3fstr = ""
        z3fstr += "from z3 import *\n"
        for sym in symTable:
            z3fstr += "%s = BitVec('%s', 64)\n" % (sym, sym)
        ...
        z3fstr += "__z3_im = Implies(__z3_x, __z3_y)\n\n"
        z3fstr += open("proof.z3.template").read()
